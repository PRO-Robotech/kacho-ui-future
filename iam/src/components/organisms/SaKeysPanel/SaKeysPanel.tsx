...
// (SAKeyService.List) + выпуск токена с TTL + одноразовый показ секрета +
...
import { iamApi, saKeysPath } from "@/api/iam";
import type { IssueSAKeyBody, IssueSAKeyResponse, ServiceAccountOAuthClient } from "@/api/iam";
...
function CreateTokenModal({
  ...
  onClose: () => void;
  onIssued: (resp: IssueSAKeyResponse) => void;
}) {
  ...
  const submit = () => {
    ...
    const body: IssueSAKeyBody = { description: description.trim(), ttl_seconds: ttlSeconds };
...
function SecretModal({ resp, onClose }: { resp: IssueSAKeyResponse; onClose: () => void }) {
...
// SaKeysPanel — таблица токенов + CTA «Создать токен» (в слоте шапки таба) +
